...
class content extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Bugs_report.Content.View');
	...
	*/
	...
	{
		$this->auth->restrict('Bugs_report.Content.Create');
		...
		if ($this->input->post('submit'))
		{
			if ($insert_id = $this->save_bugs_report())
			{
				...
				Template::redirect(SITE_AREA .'/content/bugs_report');
			}
			else
			{
	...
	*/
	...
	{
		$this->auth->restrict('Bugs_report.Content.Edit');
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/content/bugs_report');
		}
		...
		$result = $this->bugs_report_model->find($id);

		foreach ($result as $row => $rec)
		{
			foreach ($rec as $a => $b)
			$data[$a] = $b;
		}

		Template::set('bugs_report', $data);
	...
	*/
	...
	{
		$this->auth->restrict('Bugs_report.Content.Delete');
	...
	*/
	private function save_bugs_report($type='insert', $id=0)
	{
		...
		$this->form_validation->set_rules('bugs_report_URL','URL','required|unique[bf_bugs_report.bugs_report_URL,bf_bugs_report.bug_id]|trim|max_length[255]');
		...
		$data['bugs_report_Status']        = 1;
		$phpdate = time();
		$data['created_on'] 			= date( 'Y-m-d H:i:s', $phpdate );
		$data['modified_on']			= date( 'Y-m-d H:i:s', $phpdate );
